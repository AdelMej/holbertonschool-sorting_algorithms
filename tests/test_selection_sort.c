...
void test_selection_sort(void)
{
	...
	selection_sort(tabTest, 10);
	TEST_ASSERT_EQUAL_INT_ARRAY(expect, tabTest, 10);
}

void test_selection_sort_null(void)
{
	selection_sort(NULL, 10);
	TEST_ASSERT_TRUE(1);
}

void test_selection_sort_single_elem(void)
{
	int tab[1] = {10};
	int expected[1] = {10};

	selection_sort(tab, 1);	
	TEST_ASSERT_EQUAL_INT_ARRAY(expected, tab, 1);
}

void test_selection_sort_already_sorted(void)
{
...
int main(void)
{
	...
	RUN_TEST(test_selection_sort_null);
	RUN_TEST(test_selection_sort_single_elem);
	RUN_TEST(test_selection_sort_already_sorted);
	RUN_TEST(test_selection_sort_duplicate);
	RUN_TEST(test_selection_sort_reverse);
    return UNITY_END();
}
